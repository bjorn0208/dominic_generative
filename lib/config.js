...
function applyEnvKeys(config) {
  ...
  for (const envKey of Object.keys(process.env)) {
    ...
    let provider = config.providers.find((p) => p.id === id)
    if (provider) {
      provider.apiKey = process.env[envKey]
      continue
    }
    const backupMatch = id.match(/^(.+)_(\d+)$/)
    if (backupMatch) {
      provider = config.providers.find((p) => p.id === backupMatch[1])
      if (provider) provider.apiKeyBackup = process.env[envKey]
    }
  }
  return config
}
  ...
  try {
    ...
    for (const p of config.providers) {
      ...
      const updated = await client.query(
        'UPDATE providers SET name=$2, base_url=$3, api_key=$4, enabled=$5, updated_at=now() WHERE id=$1',
        [p.id, p.name, p.baseUrl || def?.baseUrl || '', p.apiKey || '', p.enabled ?? true]
      )
      if (updated.rowCount === 0) {
        await client.query(
          'INSERT INTO providers (id, name, base_url, api_key, enabled) VALUES ($1,$2,$3,$4,$5)',
          [p.id, p.name, p.baseUrl || def?.baseUrl || '', p.apiKey || '', p.enabled ?? true]
        )
      }
      for (const m of p.models || []) {
        await client.query('INSERT INTO provider_models (provider_id, model) VALUES ($1,$2)', [p.id, m])
      }
    }
    if (config.branding) {
      const updated = await client.query(
      ...
      if (updated.rowCount === 0) {
        await client.query('INSERT INTO app_config (key, value) VALUES ($1,$2)', ['branding', config.branding])
      }
    }
    if (config.huggingface) {
      ...
      if (updated.rowCount === 0) {
        await client.query('INSERT INTO app_config (key, value) VALUES ($1,$2)', ['huggingface', config.huggingface])
      }
      await client.query('DELETE FROM hf_datasets')
      for (const ds of config.huggingface.datasets || []) {
        await client.query('INSERT INTO hf_datasets (dataset_id) VALUES ($1)', [ds])
      }
    }
